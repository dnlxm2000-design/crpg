...
func _show_defeat_screen() -> void:
	...
	# center_anchor: a Control at screen center to anchor children
	...
	# Restart button (centered in an HBox)
	...
	restart_btn.horizontal_alignment = HORIZONTAL_ALIGNMENT_CENTER
	restart_btn.add_theme_font_size_override("font_size", 24)
	restart_btn.pressed.connect(_on_restart_pressed)
	hbox.add_child(restart_btn)

	# Animate in — fade dim overlay + content (CanvasLayer lacks modulate in Godot 4)
	dim.modulate = Color(1.0, 1.0, 1.0, 0.0)
	center_anchor.modulate = Color(1.0, 1.0, 1.0, 0.0)
	var tween := create_tween().set_parallel(true)
	tween.tween_property(dim, "modulate", Color(1.0, 1.0, 1.0, 1.0), 0.5)
	tween.tween_property(center_anchor, "modulate", Color(1.0, 1.0, 1.0, 1.0), 0.5)
